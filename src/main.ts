...
import express, { Express, NextFunction, Request, Response, CookieOptions } from 'express';
...
//   };
//   const redisStore = new RedisStoreWrapper(session, redisStoreOptions).getStore();
...
initCommonMiddlewares(app)
  .then((parentApp: Express) => {
    parentApp.listen(PORT, () =>
      Logger.info(
        `Running on ${PORT}`,
        `Main.ts->initCommonMiddlewares http://localhost:${PORT}/api-docs/`
      )
    );
  })
  .catch((error: any) => {
