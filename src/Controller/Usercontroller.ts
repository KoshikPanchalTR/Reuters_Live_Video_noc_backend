...
export const userList = async (req: Request, res: Response) => {
  try {
    ...
    // let RedisSavedValue: any = await getRedisValue(redisKey, true).catch((err) => {
    //   Logger.error('Prelem search : siteSettingsResponse not found in redis', 'prelemRedis', err);
...
export const CreateUser = async (req: Request, res: Response) => {
  try {
    ...
    // await setRedisValue(redisKey, usersArray, 86400, true);
...
export const testUserList = async (req: Request, res: Response) => {
  try {
    let response = [
      {
        user1: 'vaishali',
